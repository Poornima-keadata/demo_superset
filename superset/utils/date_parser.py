...
def get_since_until(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches,too-many-statements
    ...
    if (
        ...
        time_range = "DATETRUNC(DATEADD(DATETIME('today'), 0, YEAR), YEAR) : DATETIME('today') - INTERVAL 1 SECOND"
    if (
        time_range
        and time_range.startswith("current week")
        and separator not in time_range
    ):
        time_range = "DATETRUNC(DATEADD(DATETIME('today'), 0, WEEK), WEEK) : DATETIME('today') - INTERVAL 1 SECOND"
    if (
        time_range
        and time_range.startswith("current month")
        and separator not in time_range
    ):
        time_range = "DATETRUNC(DATEADD(DATETIME('today'), 0, MONTH), MONTH) : DATETIME('today') - INTERVAL 1 SECOND"
    if (
        time_range
        and time_range.startswith("current day")
        and separator not in time_range
    ):
        time_range = "DATETRUNC(DATEADD(DATETIME('today'), 0, DAY), DAY) : DATETIME('today') - INTERVAL 1 SECOND"
    if time_range and separator in time_range:
        time_range_lookup = [
            # (
            #     r"^last\s+(month|quarter|year)$",
            #     lambda unit: f"DATEADD(DATETIME('{_relative_start}'), -1, {unit})",
            # ),
            # (
            #     r"^last\s+week$",
            #     lambda : f"DATETRUNC(DATEADD(DATETIME('{_relative_start}'), -1, WEEK), WEEK) : DATETRUNC(DATEADD(DATETIME('{_relative_start}'), -1, WEEK), WEEK)",
            # ),
            # (
            #     r"^last\s+day$",
            #     lambda: f"DATEADD(DATETRUNC(DATETIME('{_relative_start}'), DAY), -1, DAY) : DATETRUNC(DATETIME('{_relative_end}'), DAY) - INTERVAL 1 DAY",
            # ),
            # (
